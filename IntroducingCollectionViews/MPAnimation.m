...
{
    return [MPAnimation renderImage:image atSize:[image size] withMargin:width color:color];
}

+ (UIImage *)renderImage:(UIImage *)image atSize:(CGSize)imageSize withMargin:(CGFloat)width color:(UIColor *)color
{
	CGSize imageSizeWithBorder = CGSizeMake(imageSize.width + 2 * (width + 1), imageSize.height + 2 * (width + 1));
	
	// Create a new context of the desired size to render the image
	UIGraphicsBeginImageContextWithOptions(imageSizeWithBorder, NO, 0);
	
	// The image starts off filled with clear pixels, so we don't need to explicitly fill them here.
	CGRect rect = CGRectMake(1, 1, imageSize.width + 2 * width, imageSize.height + 2 * width);
	[color set];
	UIRectFill(rect);
	
	[image drawInRect:(CGRect){{width + 1, width + 1}, imageSize}];
